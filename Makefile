# This Makefile is for the OS::CheckUpdates::AUR extension to perl.
#
...
#     BUILD_REQUIRES => {  }
#     CONFIGURE_REQUIRES => {  }
#     EXE_FILES => [q[bin/checkupdates-aur]]
#     INSTALLDIRS => q[site]
...
MAKE = make
FULLEXT = OS/CheckUpdates/AUR
BASEEXT = AUR
PARENT_NAME = OS::CheckUpdates
DLBASE = $(BASEEXT)
VERSION_FROM = lib/OS/CheckUpdates/AUR.pm
OBJECT = 
LDFROM = $(OBJECT)
LINKTYPE = dynamic
BOOTDEP = 
...
O_FILES  = 
H_FILES  = 
MAN1PODS = bin/checkupdates-aur
MAN3PODS = lib/OS/CheckUpdates/AUR.pm

# Where is the Config information that we are using/depend on
CONFIGDEP = $(PERL_ARCHLIBDEP)$(DFSEP)Config.pm $(PERL_INCDEP)$(DFSEP)config.h

# Where to build things
INST_LIBDIR      = $(INST_LIB)/OS/CheckUpdates
INST_ARCHLIBDIR  = $(INST_ARCHLIB)/OS/CheckUpdates
...
TO_INST_PM = lib/OS/CheckUpdates/AUR.pm

PM_TO_BLIB = lib/OS/CheckUpdates/AUR.pm \
...
DISTNAME = OS-CheckUpdates-AUR
DISTVNAME = OS-CheckUpdates-AUR-0.05
...
manifypods : pure_all  \
	bin/checkupdates-aur \
	lib/OS/CheckUpdates/AUR.pm
	$(NOECHO) $(POD2MAN) --section=1 --perm_rw=$(PERM_RW) -u \
	  bin/checkupdates-aur $(INST_MAN1DIR)/checkupdates-aur.$(MAN1EXT) 
	$(NOECHO) $(POD2MAN) --section=3 --perm_rw=$(PERM_RW) -u \
	  lib/OS/CheckUpdates/AUR.pm $(INST_MAN3DIR)/OS::CheckUpdates::AUR.$(MAN3EXT) 
...
EXE_FILES = bin/checkupdates-aur

pure_all :: $(INST_SCRIPT)/checkupdates-aur
	$(NOECHO) $(NOOP)

realclean ::
	$(RM_F) \
	  $(INST_SCRIPT)/checkupdates-aur 

$(INST_SCRIPT)/checkupdates-aur : bin/checkupdates-aur $(FIRST_MAKEFILE) $(INST_SCRIPT)$(DFSEP).exists $(INST_BIN)$(DFSEP).exists
	$(NOECHO) $(RM_F) $(INST_SCRIPT)/checkupdates-aur
	$(CP) bin/checkupdates-aur $(INST_SCRIPT)/checkupdates-aur
	$(FIXIN) $(INST_SCRIPT)/checkupdates-aur
	-$(NOECHO) $(CHMOD) $(PERM_RWX) $(INST_SCRIPT)/checkupdates-aur
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'name: OS-CheckUpdates-AUR' >> META_new.yml
	...
	$(NOECHO) $(ECHO) '   },' >> META_new.json
	$(NOECHO) $(ECHO) '   "name" : "OS-CheckUpdates-AUR",' >> META_new.json
...
pm_to_blib : $(FIRST_MAKEFILE) $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', q[$(PM_FILTER)], '\''$(PERM_DIR)'\'')' -- \
	  lib/OS/CheckUpdates/AUR.pm blib/lib/OS/CheckUpdates/AUR.pm 
